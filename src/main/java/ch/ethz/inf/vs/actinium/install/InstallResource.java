...
public class InstallResource extends LocalResource {
	...
	// The reference to the AppManager is only needed to give it to InstalledAppResource
	...
	public InstallResource(AppManager manager) {
		super(manager.getConfig().getProperty(Config.INSTALL_RESOURCE_NAME));
		
		this.manager = manager;
		
		String[] installed = getInstalledAppsName();
		for (String inst:installed) {
			AppCodeResource res = new AppCodeResource(inst, manager);
	 ...
	 */
	private void addInstalledAppResource(AppCodeResource iar) {
		add(iar);
	}
	...
	private String installAppFromString(String payload, String name) {
		...
		AppCodeResource res = new AppCodeResource(name, payload, manager);
		addInstalledAppResource(res);
